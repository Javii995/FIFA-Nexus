...
interface QuizQuestion {
    type: 'club' | 'league';
    correctAnswer: ClubData | LeagueData;
    options: (ClubData | LeagueData)[];
}

// Tijdelijke opslag voor actieve quiz sessies
const activeQuizSessions = new Map<string, {
...
const FALLBACK_LEAGUES: LeagueData[] = [
    {
        id: '1',
        name: 'Premier League',
...
const getRandomClubs = async (count: number = 20): Promise<ClubData[]> => {
    try {
        ...
        console.log('API Response ontvangen, items:', data.items?.length || 0);
    ...
    try {
        ...
        if (isClubQuestion) {
            ...
            if (availableClubs.length < 4) {
                ...
                if (allAvailable.length < 4) return null;
...
export const saveQuizScore = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const userId = req.user?.id;
...
export const getCurrentQuizSession = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const userId = req.user?.id;
